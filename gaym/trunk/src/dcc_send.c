 ...
 */
static void gaym_dccsend_recv_ack(GaimXfer *xfer, const char *data, size_t size) {
	unsigned long l;
...
ssize_t gaym_dccsend_send_write(const char *buffer, size_t size, GaimXfer *xfer)
{
	ssize_t s;
...
static void gaym_dccsend_send_connected(gpointer data, int source, GaimInputCondition cond) {
	...
	xd->inpa = gaim_input_add(conn, GAIM_INPUT_READ, gaym_dccsend_send_read, xfer);
	/* Start the transfer */
