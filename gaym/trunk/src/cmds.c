...
{
	GaimConversation *convo = gaim_find_conversation_with_account(target, gaym->account);
	char *buf;
...
int gaym_cmd_invite(struct gaym_conn *gaym, const char *cmd, const char *target, const char **args)
{
	...
	buf = gaym_format(gaym, "vnc", "INVITE", args[0], args[1] ? args[1] : target);
	gaym_send(gaym, buf);
	g_free(buf);
...
int gaym_cmd_mode(struct gaym_conn *gaym, const char *cmd, const char *target, const char **args)
{
	...
	if (!strcmp(cmd, "mode")) {
		...
		buf = gaym_format(gaym, "vnv", "MODE", gaim_connection_get_display_name(gc), args[0]);
	} else {
		return 0;
	}
...
int gaym_cmd_part(struct gaym_conn *gaym, const char *cmd, const char *target, const char **args)
{
	...
	if (args[1])
		buf = gaym_format(gaym, "vc:", "PART", args[0] ? args[0] : target, args[1]);
	else
...
int gaym_cmd_quit(struct gaym_conn *gaym, const char *cmd, const char *target, const char **args)
{
	...
	if (!gaym->quitting) {
		buf = gaym_format(gaym, "v:", "QUIT", (args && args[0]) ? args[0] : "Download Gaim: " GAIM_WEBSITE);
		gaym_send(gaym, buf);
		g_free(buf);
...
int gaym_cmd_remove(struct gaym_conn *gaym, const char *cmd, const char *target, const char **args)
{
	...
	if (args[1])
		buf = gaym_format(gaym, "vcn:", "REMOVE", target, args[0], args[1]);
	else
...
static void gaym_do_mode(struct gaym_conn *gaym, const char *target, const char *sign, char **ops)
{
	...
	while (ops[i]) {
		if (ops[i + 2] && ops[i + 4]) {
			...
			buf = gaym_format(gaym, "vcvn", "MODE", target, mode, ops[i + 1]);
			i += 2;
		}
		gaym_send(gaym, buf);
		g_free(buf);
