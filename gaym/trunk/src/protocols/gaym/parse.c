...
static GaimCmdRet gaym_parse_gaim_cmd(GaimConversation *conv, const gchar *cmd,
                                        gchar **args, gchar **error, void *data)
{
	...
	(cmdent->cb)(gaym, cmd, gaim_conversation_get_name(conv), (const char **)args);
...
static char *gaym_send_convert(struct gaym_conn *gaym, const char *string)
{
	...
	charset = gaim_account_get_string(gaym->account, "encoding", IRC_DEFAULT_CHARSET);
	...
	if (err) {
		...
		gaim_debug(GAIM_DEBUG_ERROR, "gaym", "Sending as UTF-8 instead of %s\n", charset);
...
char *gaym_parse_ctcp(struct gaym_conn *gaym, const char *from, const char *to, const char *msg, int notice)
{
	...
	if (!strncmp(cur, "ACTION ", 7)) {
		...
		buf = gaym_format(gaym, "vt:", "NOTICE", from, "\001VERSION Gaim IRC\001");
		gaym_send(gaym, buf);
		g_free(buf);
...
void gaym_msg_table_build(struct gaym_conn *gaym)
{
	...
	if (!gaym || !gaym->msgs) {
		gaim_debug(GAIM_DEBUG_ERROR, "gaym", "Attempt to build a message table on a bogus structure\n");
		return;
	}

	for (i = 0; _gaym_msgs[i].name; i++) {
		g_hash_table_insert(gaym->msgs, (gpointer)_gaym_msgs[i].name, (gpointer)&_gaym_msgs[i]);
	}
}
...
void gaym_cmd_table_build(struct gaym_conn *gaym)
{
	...
	if (!gaym || !gaym->cmds) {
		gaim_debug(GAIM_DEBUG_ERROR, "gaym", "Attempt to build a command table on a bogus structure\n");
		return;
	}

	for (i = 0; _gaym_cmds[i].name ; i++) {
		g_hash_table_insert(gaym->cmds, (gpointer)_gaym_cmds[i].name, (gpointer)&_gaym_cmds[i]);
	}
}
...
void gaym_parse_msg(struct gaym_conn *gaym, char *input)
{
	...
	if (!strncmp(input, "PING ", 5)) {
		...
		gaim_connection_error(gaim_account_get_connection(gaym->account), _("Disconnected."));
		return;
	}
	...
	for (cur = end, fmt = msgent->format, i = 0; fmt[i] && *cur++; i++) {
		...
		case '*':
			...
			break;
		default:
			gaim_debug(GAIM_DEBUG_ERROR, "gaym", "invalid message format character '%c'\n", fmt[i]);
			break;
		}
	}
